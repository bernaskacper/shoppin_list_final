const CACHE_NAME = 'shopping-list-v3'; // ZMIENIONO Z v2 NA v3!
const urlsToCache = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .then(() => {
        console.log('Service Worker v3: Cached all files successfully');
        return self.skipWaiting(); // Immediately activate new version
      })
      .catch(error => {
        console.error('Service Worker v3: Caching failed', error);
      })
  );
});
  ...
  event.respondWith(
    caches.match(event.request)
      .then(function(response) {
        ...
        if (response) {
          console.log('Service Worker v3: Serving from cache', event.request.url);
          return response;
        }
        ...
        return fetch(fetchRequest).then(
          ...
          console.log('Service Worker v3: Fetch failed, serving offline page');
...
self.addEventListener('activate', function(event) {
  ...
  event.waitUntil(
    caches.keys().then(function(cacheNames) {
      ...
      console.log('Service Worker v3: Activated successfully');
...
self.addEventListener('sync', function(event) {
  if (event.tag === 'background-sync') {
    console.log('Service Worker v3: Background sync triggered');
...
self.addEventListener('message', function(event) {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('Service Worker v3: Received SKIP_WAITING message');
    self.skipWaiting();
  }
});